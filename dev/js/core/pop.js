define(['core/cncnERP'], function(cncnERP){
	...
	/**
	* cncnERP.ui.pop 弹出窗控件
	* @class cncnERP.ui.pop
	* @author norion.z
	...
	var Pop = function(o){
		var g      = this,
			...
			*/
			_core  = {
					...
					if( title ){
						...
						if( allowClose ){
							...
							if( !popMain.find('.l-pop-close').length ){
								popMain.prepend('<div class="l-pop-close"><i title="关闭"></i></div>');
							}
					...
					if( url ){
						...
						$.ajax({
							...
							success : function(data){
                                if( cncnERP.base.isFunction(ajaxSuccess) ){
                                    ajaxSuccess(data);
                                }
					...
					if( btns ){
						...
						btnMain
							...
							.on('click', 'a', function(){
								...
								cncnERP.base.isFunction(item.onclick) && item.onclick.apply(this, [id, i, item, that]);
								isClose && g.close(id);
							});
				...
				*/
				loadFn: function(){
					if( cncnERP.base.isFunction(p.onloadFn) ){
		...
		*/
		g.close = function(id){
			...
			/*关闭时要触发的事件*/
			if( cncnERP.base.isFunction(closeFn) ){
		...
		/**
		* 修改标题
		* @method cncnERP.ui.pop.modifyTitle
		...
		*/
		...
		/**
		* 修改按钮
		* @method cncnERP.ui.pop.modifyBtns 
		...
		*/
		...
		/**
		* 修改窗体大小
		* @method cncnERP.ui.pop.modifyWrap 
		* @param {Number} width 
		* @param {Number} height
		*/

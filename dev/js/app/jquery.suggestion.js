    ...
    var defaults      = null,
        suggestion    = function(defaults, self, selfIndex){
            var width          = defaults.width,
                pregKind       = defaults.pregKind,
                ajax           = defaults.ajax,
                isTouchRequest = defaults.isTouchRequest,
                allData        = [],
                filterData     = [],
                isClose        = true,
                initValue      = defaults.initValue ? defaults.initValue : '',
            ...
            if (typeof defaults.name == 'undefined') {
                name = selfName + 'Hidden';
            } else if (typeof defaults.name == 'string') {
                name = defaults.name;
            } else if (typeof defaults.name == 'object') {
                name = defaults.name[selfIndex] ? defaults.name[selfIndex] : selfName + 'Hidden';
            }
            var value          = defaults.value.length ? (defaults.value[selfIndex] ? defaults.value[selfIndex] : '') : '',
                ...
                showFn         = function(e){
                    ...
                    if( isTouchRequest ){
                        ...
                        for(var i = 0, len = allData.length; i<len; i++){
                            ...
                            if( str.toLocaleLowerCase().indexOf(val.toLocaleLowerCase()) !== -1 && pregKind === 1){
                                data.push(item);
                            }
                            if( str.toLocaleLowerCase().indexOf(val.toLocaleLowerCase()) === 0 && pregKind === 2){
                                data.push(item);
                            }
                        }
                    }
                ...
                enterEvent     = function(data, target){
                    ...
                    defaults.touch.apply(self, [data, self, hiddenInput]);
                    if( !data.name ){
            ...
            self.attr('autocomplete', 'off')
                ...
                .on('blur', function(){
                    isClose && closeFn();
                    if( defaults.isResultNullBlurClear ){
                        if( !filterData.length ){
                            self.val('');
                            hiddenInput.val('');
                            defaults.blurClear.apply(self, [self, hiddenInput]);
                        }else{
                            if( !hiddenInput.val() ){
                                self.val('');
                                defaults.blurClear.apply(self, [self, hiddenInput]);
            ...
            if( initValue )self.val(initValue);
            ...
            listWrap
                ...
                .on('click', 'li', function(e){
                    ...
                    if (! defaults.autoClose) {
                        self.trigger('focus').trigger('blur');
                    }
                    closeFn();
                })
